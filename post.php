...
class catalogFill {
    ...
    function getChildCategoriesID($parent_id){
        ...
        if(count($childrens)>0){
            $sql = "SELECT id, pagetitle FROM ".$this->modx->getFullTableName('site_content')." WHERE id IN (".implode(',',$childrens).") AND isfolder = 1;";
        ...
        if(file_exists($file_path)){
            ...
            for ($row = $start; $row <= $highestRow; $row++) {
                ...
                foreach($this->config['content_row'] as $col => $v){
                    ...
                    $c_value = $this->modx->db->escape(str_replace('\=','=',$c_value));
    ...
    function getChildCategories($parent_id){
        ...
        if(count($childrens)>0){
            $sql = "SELECT id, pagetitle, parent FROM ".$this->modx->getFullTableName('site_content')." WHERE id IN (".implode(',',$childrens).") AND isfolder = 1;";
            $rs = $this->modx->db->query($sql);
            if($this->modx->db->getRecordCount($rs)>0){
                while($row = $this->modx->db->getRow($rs)){
    ...
    function setParentCategoryExport($parent_id,$index){
        ...
        foreach($this->categories as $key => $val){
            if(in_array($parent_id, array_keys($val))){
                ...
                $parent_id = $key;
                if($index > $count){
                    $name = $this->setParentCategoryExport($parent_id,$index-1);
                }
                $count++;
            }
        }
